...
final class ContactStore {
    ...
    func save(_ contact: Contact) throws {
        let stmt = try db.prepare("""
            ...
            (id, label, publicKey, identityKey, ratchetState, previousKey, fallbackKey, pushToken, rotationCounter, sessionCount, createdAt, lastSessionAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        """)
    ...
    func fetchAll() throws -> [Contact] {
        let stmt = try db.prepare("SELECT * FROM contacts ORDER BY lastSessionAt DESC, createdAt DESC;")
        defer { sqlite3_finalize(stmt) }
    ...
    func fetch(id: UUID) throws -> Contact? {
        let stmt = try db.prepare("SELECT * FROM contacts WHERE id = ?;")
        defer { sqlite3_finalize(stmt) }
    ...
    func fetchByIdentityKey(_ identityKey: Data) throws -> Contact? {
        let stmt = try db.prepare("SELECT * FROM contacts WHERE identityKey = ?;")
        defer { sqlite3_finalize(stmt) }
    ...
    func delete(id: UUID) throws {
        ...
        let skStmt = try db.prepare("DELETE FROM skippedKeys WHERE contactId = ?;")
        defer { sqlite3_finalize(skStmt) }
        sqlite3_bind_text(skStmt, 1, idStr, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
        sqlite3_step(skStmt)

        // Delete contact
        let stmt = try db.prepare("DELETE FROM contacts WHERE id = ?;")
        defer { sqlite3_finalize(stmt) }
        sqlite3_bind_text(stmt, 1, idStr, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
    ...
    func deleteAll() throws {
        try db.execute("DELETE FROM skippedKeys;")
        try db.execute("DELETE FROM contacts;")
    }
    ...
    func updateLabel(contactId: UUID, label: String) throws {
        let stmt = try db.prepare("UPDATE contacts SET label = ? WHERE id = ?;")
        defer { sqlite3_finalize(stmt) }
    ...
    func saveSkippedKeys(contactId: UUID, keys: [(dhPublicKey: Data, messageNumber: Int, messageKey: Data)]) throws {
        ...
        // Delete existing
        let delStmt = try db.prepare("DELETE FROM skippedKeys WHERE contactId = ?;")
    ...
    func fetchSkippedKeys(contactId: UUID) throws -> [(dhPublicKey: Data, messageNumber: Int, messageKey: Data)] {
        let stmt = try db.prepare("SELECT dhPublicKey, messageNumber, messageKey FROM skippedKeys WHERE contactId = ?;")
        defer { sqlite3_finalize(stmt) }
    ...
    ///               identityKey(9), ratchetState(10), sessionCount(11)
