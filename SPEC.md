## АВТОМАТИЧЕСКАЯ ВЕРИФИКАЦИЯ — ОБЯЗАТЕЛЬНО

В конце КАЖДОЙ фазы ты обязан:

1. Создать скрипт verify_phase_N.sh
2. Запустить его сам
3. Если вернул не-ноль — найти ошибку, исправить, запустить снова
4. Повторять пока не вернёт 0
5. Только после этого переходить к следующей фазе

ТЫ НЕ СПРАШИВАЕШЬ МЕНЯ "проверь работает ли".
ТЫ САМ ПРОВЕРЯЕШЬ через скрипты.

Если что-то невозможно проверить автоматически (например требуется
физический iPhone) — четко скажи "следующее требует ручной проверки
на устройстве: [список]" и продолжай с тем что можно проверить.
